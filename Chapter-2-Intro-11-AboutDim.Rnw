...
if (lan == 'en') {
  ...
  question.2 <- 'If we executed in R the commands \\verb|dim(x)[1]|, \\verb|ncol(x)| and \\verb|length(x)| in that exact order, what would be the result?'
...
if (lan == 'pt-br') {
  ...
  question.2 <- 'Caso executássemos os comandos  \\verb|dim(x)[1]|, \\verb|ncol(x)| e \\verb|length(x)|, nessa mesma ordem, qual seria o resultado?'
}
